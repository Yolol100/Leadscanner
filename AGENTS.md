...
For normal lead work, use `filter_core_v17` / source set `17.2.0-compliance-gate` with Project Leads `leadpromo.md` V17.3.1:

1. verify a real company and official site, or use the bounded `website_absent` exception only when a current official business profile proves the company and directly proves no website link is present;
...
6. verify a public business email from an official source and record `contact_verified`; this is contact evidence only, never outreach permission;
7. apply the separate fail-closed compliance gate. Default is `COMPLIANCE_NOT_PROVEN`; only a documented Gate 1 consent, Gate 2 explicit designation/purpose match, or Gate 3 existing-customer-similar route may produce `COMPLIANCE_PASSED`;
8. never auto-clear generic public addresses such as `info@`; Instantly/enrichment/email verification cannot override the compliance gate;
9. write one short curiosity-first permission-CTA email through `scripts/outreach_copy_v17_3.py`; claim an example already exists only when artifact existence and readback are proven, otherwise offer to make it;
10. pass the compliance, V17.3 copy and suppression gates before mailbox or Instantly staging;
11. create at most one deduplicated mijn.host IMAP draft and read it back exactly once when the selected route is draft creation.
