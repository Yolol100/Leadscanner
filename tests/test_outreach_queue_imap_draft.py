...
from outreach_queue_imap_draft import resolve_queue_row, suppression_sets, validate_queue_row


class QueueImapDraftTests(unittest.TestCase):
    def test_resolve_requires_one_exact_lead(self):
        values = [
            ["lead_id", "email", "status"],
            ["lead-1", "one@example.com", "manual_review"],
        ]
    ...
    def test_suppression_sets_are_normalized(self):
        values = [
            ["email", "domain", "reason"],
            ["Stop@Example.com", "Blocked.test", "optout"],
        ]
    ...
    def test_valid_manual_review_row_can_be_drafted(self):
        row = {
            ...
            "subject": "Quote requests at Example",
            "body": "Hello",
        ...
        self.assertEqual(
            validate_queue_row(row, sender_email="info@andrewbaeten.nl"),
            [],
        )
    ...
    def test_draft_blocks_suppressed_or_already_sent_rows(self):
        ...
        errors = validate_queue_row(
            row,
            sender_email="info@andrewbaeten.nl",
            suppressed_domains={"example.com"},
        )
