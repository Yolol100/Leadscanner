...
class OutreachAgentPrepareV2Tests(unittest.TestCase):
    def test_nl_human_copy_has_process_subject_no_url_no_ai_subject(self):
        ...
        self.assertIn("kleine AI-ondersteunde workflow", body)
    ...
    def test_en_quote_copy_is_human_and_us_footer_is_at_signature(self):
        ...
        self.assertIn("customers answer a few straightforward questions", body)
        self.assertNotIn("AI Quote & Intake Agent", body)
        self.assertNotIn("bounded digital agents", body)
        self.assertNotIn("human handoff", body)
    ...
    def test_cta_b_changes_only_permission_wording(self):
        common = dict(company="Voorbeeld Tandarts", country="NL", fact="Op jullie website kunnen bezoekers direct een afspraak maken.", idea="legacy value", agent_type="front_desk_sales")
        with patch.dict(os.environ, {"OUTREACH_CTA_VARIANT": "A"}, clear=False):
            a = build_copy(**common)
    ...
    def test_payload_contains_human_value_asset_and_campaign_target(self):
        ...
        self.assertNotIn("AI Front Desk", meta["value_asset_summary"])
        self.assertEqual(meta["cta_variant"], "A")
        self.assertEqual(meta["copy_contract"], "human_v13_4")
