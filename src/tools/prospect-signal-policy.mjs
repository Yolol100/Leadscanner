...
export function classifyProspectSignal(signal = {}) {
  ...
  if (type === 'website_absent') {
    const pass = Boolean(signal.officialBusinessProfile && signal.websiteLinkAbsent === true && signal.publicBusinessEmailVerified === true);
    return pass
  ...
  if (DISCOVERY_ONLY.has(type)) return { decision: 'prioritize_discovery', offer: null, evidenceClass: 'discovery_only' };
  if (type === 'missing_llms_txt' || type === 'broken_llms_txt') return { decision: 'not_search_evidence', offer: null, evidenceClass: 'non_qualifying' };
